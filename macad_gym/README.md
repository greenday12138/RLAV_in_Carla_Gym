...
1. Setup CARLA (0.9.x)
    ...
    3.1  Create a folder for Carla installation, here we denote it as SOFTWARE 
    `mkdir ~/SOFTWARE && cd ~/SOFTWARE`

    3.2 Example: Download the 0.9.14 release version from: [Here](https://github.com/carla-simulator/carla/releases)
    Extract it into `~/SOFTWARE`
    
    3.3 `echo "export CARLA_SERVER=${HOME}/SOFTWARE/CarlaUE4.sh" >> ~/.bashrc`

    > NOTE: Restart the system to refresh environment variables. (We only test above codes on Ubuntu 22.04 and Carla 0.9.14.)

1. Install MACAD-Gym:
   - **Option for developers**:
     - Create a new conda env named "macad-gym", set python==3.7.0 
     - Activate the `macad-gym` conda python env:
      `source activate macad-gym`
     - `cd `***{Your Repo Address Here}***`/RLAV_in_Carla_Gym/macad_gym` Install the `macad-gym` package:
	  `pip install -e .`
     - Install CARLA PythonAPI: `pip install carla==0.9.14`
