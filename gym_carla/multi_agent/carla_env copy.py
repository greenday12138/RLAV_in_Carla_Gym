...
class CarlaEnv:
    ...
    def reset(self):
        ...
        self._ego_autopilot(self.ego_vehicle,True)
    ...
    def _speed_switch(self,a_index):
        ...
        if self.speed_state == SpeedState.START:
            ...
            if ego_speed >= self.speed_threshold:
                ...
                self._ego_autopilot(self.ego_vehicle,False)
                if not self.RL_switch:
    ...
    def _ego_autopilot(self,ego_veh, setting=True):
        # Use traffic manager to control ego vehicle
        ego_veh.set_autopilot(setting, self.tm_port)
        if setting:
            speed_diff = (30 - self.speed_limit) / 30 * 100
            self.traffic_manager.distance_to_leading_vehicle(ego_veh, self.min_distance)
            if self.ignore_traffic_light:
                self.traffic_manager.ignore_lights_percentage(ego_veh, 100)
                self.traffic_manager.ignore_walkers_percentage(ego_veh, 100)
            self.traffic_manager.ignore_signs_percentage(ego_veh, 100)
            self.traffic_manager.ignore_vehicles_percentage(ego_veh, 0)
            self.traffic_manager.vehicle_percentage_speed_difference(ego_veh, speed_diff)
            if self.auto_lanechange and self.speed_state == SpeedState.RUNNING:
                self.traffic_manager.auto_lane_change(ego_veh, True)
                self.traffic_manager.random_left_lanechange_percentage(ego_veh, 100)
                self.traffic_manager.random_right_lanechange_percentage(ego_veh, 100)


            # self.traffic_manager.set_desired_speed(ego_veh, 72)
            # ego_wp=self.map.get_waypoint(ego_veh.get_location())
            # self.traffic_manager.set_path(ego_veh,path)
            ...
            self.traffic_manager.set_route(ego_veh,
