"""Attention: This client only work on synchronous mode"""
...
class EgoClient:
    def __init__(self, args,opt) -> None:
        ...
        logging.info('Client %s listening to server %s:%s', self.id,args.host, args.port)
        ...
        logging.info('Client %s connected to Carla Server!',self.id)
    ...
    def __del__(self):
        logging.info(f"\n Destroying Client {self.id}")

    def reset_before_tick(self):
        logging.info(f"CLIENT {self.id} RESET BEFORE TICK")
