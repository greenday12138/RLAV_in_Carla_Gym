...
from algs.pdqn import PolicyNet_multi, PriReplayBuffer
...
class veh_lane_encoder(torch.nn.Module):
    def __init__(self, state_dim, train=True):
        ...
        self.npc_encoder = nn.Linear(state_dim['companion_vehicle'] * 2, self.hidden_size)
        self.light_encoder = nn.Linear(state_dim['light'], self.hidden_size)
        self.hero_encode = nn.Linear(state_dim['hero_vehicle'], self.hidden_size)
...
class MAPDQN:
    def __init__(self, state_dim, action_dim, action_bound, gamma, tau, sigma, sigma_steer,
                 sigma_acc, theta, epsilon, buffer_size, batch_size, actor_lr, critic_lr,
                 clip_grad, zero_index_gradients, inverting_gradients, per_flag, device, 
                 agent_names:list):
        self.agents = {}
        for name in agent_names:
            self.agents.update({
                name: P_DQN(state_dim, action_dim, action_bound, gamma, tau, sigma, 
                    sigma_steer, sigma_acc, theta, epsilon, buffer_size, batch_size, 
                    actor_lr, critic_lr, clip_grad, zero_index_gradients, 
                    inverting_gradients, per_flag, device)
                })
    ...
    def take_action(self, states:dict):
        actions = {}
        for name in self.agents.keys():
            actions.update({
                name: self.agents[name].take_action(states[name])
            })
